...
class AI:
    ...
    def get_metrics_vector(self, metrics):
        # Normalize heuristic metrics for neural network input range
        return [
    ...
    def get_best_move(self):
        # Exploration logic: Select random rotation and position
        if random.random() < self.agent.epsilon:
            ...
            if grid_after:
                ...
                self.last_state_vector = self.get_metrics_vector(metrics)
        ...
        # Evaluation logic: Simulate all possible moves and query model for state value
    ...
    def simulate_move(self, start_grid, block, x):
        # Execute virtual drop to determine final state of the board
    ...
    def collect_experience(self, next_grid, reward, done):
        # Package transition data and update replay buffer for training
